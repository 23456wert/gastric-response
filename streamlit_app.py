...
# SHAP参数
DEFAULT_FORCE_TOP_N = 10
DEFAULT_PLOT_TOP_N = 15
SHAP_NSAMPLES = 160
BACKGROUND_N = 30
...
def infer_feature_group(feature_name: str) -> str:
    """
    仅分两组：
    1) Venous-phase CT Features
    2) Elasticity Features
    """
    fname = str(feature_name).strip()

    if fname.startswith("Elasticity_") or fname.startswith("Elasticityperi_"):
        return "Elasticity Features"
    elif fname.startswith("Venous_") or fname.startswith("Venousperi_"):
        return "Venous-phase CT Features"
    else:
        # 若仍有其它前缀，为了部署时更稳，默认按静脉期CT放入 Other
        return "Other Features"

def format_widget_label(name: str, max_len: int = 60) -> str:
    """
    输入区标签：尽量保留识别度
    """
    s = str(name)
    s = s.replace("Venousperi_", "Venous-Peri · ")
    s = s.replace("Elasticityperi_", "Elasticity-Peri · ")
    s = s.replace("Venous_", "Venous-Intra · ")
    s = s.replace("Elasticity_", "Elasticity-Intra · ")
...
def format_force_label(name: str, max_len: int = 36) -> str:
    """
    force plot 专用，必须短
    """
    s = str(name)
...
def subset_explanation(explanation, top_n=12, for_force=False):
    ...
    if for_force:
        ...
        display_names = [format_widget_label(n, max_len=58) for n in top_names]
...
def build_shap_table(explanation, top_n=None):
    values = np.array(explanation.values)
    names = np.array(explanation.feature_names)
    data = np.array(explanation.data)

    if top_n is None or top_n >= len(values):
        order = np.argsort(np.abs(values))[::-1]
    else:
        order = np.argsort(np.abs(values))[::-1][:top_n]

    df = pd.DataFrame({
        "Feature Group": [infer_feature_group(n) for n in names[order]],
        "Full Feature Name": names[order],
        "Display Name": [format_widget_label(n, max_len=68) for n in names[order]],
        "Input Value": data[order],
        "SHAP Value": values[order],
        "Absolute SHAP": np.abs(values[order]),
        "Direction": np.where(values[order] >= 0,
...
def plot_waterfall(explanation, max_display=15):
    ...
    plt.figure(figsize=(11.5, 7.2), dpi=300)
...
def make_input_widgets(feature_meta, feature_names):
    ...
    for group_name in group_order:
        ...
        with st.expander(f"{group_name} ({len(feats)})", expanded=True):
            cols = st.columns(3)
            for idx, feat in enumerate(feats):
                ...
                help_text = (
                    ...
                    f"Training range: {meta['min']:.6f} to {meta['max']:.6f}\n"
                    f"Median: {meta['median']:.6f}\n"
                    f"IQR: {meta['q1']:.6f} - {meta['q3']:.6f}"
                )

                with col:
                    user_values[feat] = st.number_input(
                        label=format_widget_label(feat, max_len=60),
                        min_value=min_v,
                        max_value=max_v,
                        value=default_v,
                        help=help_text,
...
with st.sidebar:
    ...
    st.write(f"**Feature grouping:** Venous-phase CT / Elasticity")
    st.markdown("---")
    force_top_n = st.slider(
        "Force plot display count",
        min_value=5,
        max_value=min(25, len(feature_names)),
        value=min(DEFAULT_FORCE_TOP_N, len(feature_names)),
        step=1
    )
    plot_top_n = st.slider(
        "Waterfall / bar plot display count",
        min_value=5,
        max_value=min(40, len(feature_names)),
        value=min(DEFAULT_PLOT_TOP_N, len(feature_names)),
        step=1
    )
    show_all_shap_table = st.checkbox("Show all features in SHAP table", value=True)
    st.markdown("---")
...
with st.form("prediction_form", clear_on_submit=False):
    st.markdown(
        ...
        'Enter patient-specific radiomics feature values below. '
        'All model features are displayed and grouped into Venous-phase CT features and Elasticity features.'
        '</div>',
        unsafe_allow_html=True
    )
...
if preview and not submitted:
    with st.expander("Current Input Table", expanded=True):
        preview_df = input_df.T.rename(columns={0: "Value"})
        preview_df.index.name = "Feature"
...
if submitted:
    ...
    with st.expander("Submitted Feature Values", expanded=False):
        submitted_df = input_df.T.rename(columns={0: "Value"})
        submitted_df.index.name = "Feature"
    ...
    with st.spinner("Computing SHAP explanation..."):
        ...
        plot_exp = subset_explanation(explanation_full, top_n=plot_top_n, for_force=False)

        if show_all_shap_table:
            shap_df = build_shap_table(explanation_full, top_n=None)
        else:
            shap_df = build_shap_table(explanation_full, top_n=plot_top_n)

    tab1, tab2, tab3 = st.tabs([
        f"SHAP Force Plot (Top {force_top_n})",
        f"Waterfall / Contribution Plot (Top {plot_top_n})",
        "SHAP Feature Table"
    ])
    ...
    with tab1:
        st.caption(
            ...
            f"Displaying all features in a force plot usually causes severe overlap, so only the most important features are shown here."
        )
        render_force_plot_html(force_exp, height=320)

        st.info(
            "Input section shows all model features, and the SHAP table can also display all features. "
            "However, the force plot is intentionally limited to the top-ranked features to preserve readability."
        )

    with tab2:
        st.caption(
            f"Waterfall plot and signed SHAP bar chart for the top {plot_top_n} contributing features."
        )
        fig1 = plot_waterfall(plot_exp, max_display=plot_top_n)
        st.pyplot(fig1, use_container_width=True)

        fig2 = plot_top_contrib_bar(plot_exp, max_display=plot_top_n)
        st.pyplot(fig2, use_container_width=True)

    with tab3:
        if show_all_shap_table:
            st.caption("All features ranked by absolute SHAP magnitude.")
        else:
            st.caption(f"Top {plot_top_n} features ranked by absolute SHAP magnitude.")
...
Research-use interface for the multimodal VE-EF model.<br>
All input features are displayed in the prediction form and grouped into venous-phase CT features and elasticity features.
SHAP tables can display all features, while dense visual plots are restricted to the most influential features to maintain readability.
</div>
""", unsafe_allow_html=True)
